...
def main():
    print("=" * 60)
    print("RUNNING AUTOMATED QUANTITY KEYBOARD INPUT PHASE 03 TESTS")
    print("=" * 60)
    ...
    try:
        ...
        print(res.stdout)
        if res.returncode == 0:
            log_success("All Flutter unit, widget, and integration tests passed!")
        else:
            print(res.stderr)
            log_failure(f"Flutter test suite failed with exit code {res.returncode}")
    except Exception as e:
        log_failure(f"Failed to execute Flutter test suite: {e}")
        
    print("=" * 60)
    log_success("PHASE 03 COMPLETED SUCCESSFULLY!")
    print("=" * 60)

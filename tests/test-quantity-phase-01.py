...
def main():
    print("=" * 60)
    print("RUNNING AUTOMATED QUANTITY SELECTOR PHASE 01 UNIT TESTS")
    print("=" * 60)
    
    flutter_bin = find_flutter()
    workspace_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    
    test_file = os.path.join(workspace_dir, "test", "quantity_selector_test.dart")
    ...
    try:
        res = subprocess.run(
            [flutter_bin, "test", "test/quantity_selector_test.dart"],
            cwd=workspace_dir,
            capture_output=True,
            text=True
        )
        print(res.stdout)
        if res.returncode == 0:
            log_success("All QuantitySelector unit tests passed successfully!")
        else:
            print(res.stderr)
